...
mem_init(void)
{
	...
	size_t pages_array_size = npages * sizeof(struct PageInfo);
	boot_map_region(kern_pgdir, UPAGES, pages_array_size, PADDR(pages), PTE_U);
...
mem_init_mp(void)
{
	...
	for (int i = 0; i < NCPU; i++) {
		...
		boot_map_region(kern_pgdir,
		                (uintptr_t) kstacktop_i - KSTKSIZE,
		                KSTKSIZE,
		                PADDR(&percpu_kstacks[i][0]),
		                PTE_P | PTE_W);
	}
}
...
page_remove(pde_t *pgdir, void *va)
{
	// Fill this function in
	pte_t *pg_entry;
	struct PageInfo *page = page_lookup(pgdir, va, &pg_entry);
	if (page == NULL)
		return;
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	boot_map_region(
	        kern_pgdir, base, roundup_size, pa, PTE_PCD | PTE_PWT | PTE_W);
...
{
	// LAB 3: Your code here.
	for (int i = (uint32_t) va; i < (uint32_t) ROUNDUP(va + len, PGSIZE);
	     i += PGSIZE - (i % PGSIZE)) {
		if ((uint32_t) i > ULIM) {
